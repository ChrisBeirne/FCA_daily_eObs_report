---
title: "FCA: Daily movement report"
author:
  - Chris Beirne
...
This document summarizes the movement locations for all active eObs devices in FCA's `Ecología del movimiento S. papa` project. 
...
# Get animals
animals <- getMovebankAnimals(study=2885579493,login=loginStored)

# Move nickname to locak_identifier
animals$local_identifier <- animals$nick_name
animals$animalName <- animals$nick_name
...
ec_names <- data.frame(code=row.names(mov_dat@idData), name=(mov_dat@idData$nick_name))
...
# Convert timezone
mov_dat$timestamp <- with_tz(timestamps(mov_dat), tz="America/Guayaquil")
...
if(nrow(tmp2)>0){
  tmp2[, c("name", "timestamp_end", "days_since_check_in")] %>%
    kbl() %>% kable_styling() %>% kableExtra::scroll_box(width = "100%")
  } else {print("None")}
```

**Tagging summary**

The proportion of active tags by species:
...
Average hourly schedule of the tags:
...
# Recent locations (last 2 weeks)  {.tabset}

The following maps relate to just the last two weeks.  

```{r, echo=F, message=F, warning=F}
# Subset to 48 hours
tmp_ecuador_48 <- dat[dat$country=="ecuador" & dat$timestamp > max(ymd_hms(animals$timestamp_end),na.rm=T)-14*24*60*60,]
...
The following map shows all of the vulture clusters which have occurred in the previous week. This map should be checked daily to find potential feeding sites for visual confirmation. The field team should:
...
- The colors denote how recently the cluster started (Red >24 hours; orange 24-48 hours; blue 48+ hours)
...
# Just pull last 7 days
start_t2 <- t-as.difftime(7,units='days')
...
sub_dat_df <- left_join(sub_dat_df, animals[, c("tag_id", "animalName", "taxon_canonical_name")])
...
#table(sub_dat_df$local_identifier)
#table(is.na(sub_dat_df$location_lat))
#table(is.na(sub_dat_df$location_long))


lfc <- do.call(st_sfc,
              lapply(split(sub_dat_df, as.character(sub_dat_df$name)),
                     function(d){st_linestring(as.matrix(d[,c("location_long", "location_lat")]))}))
...
# add back in the metadata
dat_shp  <- left_join(dat_shp,sp_dat)
dat_shp$name <- gsub("^.*\\.","", dat_shp$name )
# Bring in the colours from earlier
ecuador_cols <- tmp_ecuador[, c("name", "colours")]
ecuador_cols <- ecuador_cols[duplicated(ecuador_cols)==F,]
dat_shp <- left_join(dat_shp, ecuador_cols)
dat_shp$country <- "ecuador"
dat_shp$taxon_canonical_name <- "Sarcoramphus papa"
...
#            logger.info("One of your tracks contains only one location, so between-location speed cannot be calculated. The location is kept in the data set, but might corrupt the results.")
    ...
    data.night <- foreach(data.groundi = data.ground.split) %do% {
      ...
      #data.groundi <- data.ground.split[[1]]
...
The following map shows the locations where birds spend the night for the last two weeks. 
...
# Just pull last 7 days
start_t2 <- t-as.difftime(14,units='days')
sub_dat <- sub_dat[sub_dat$timestamp>start_t2,]


sub_dat$name <- gsub("^.*\\.","", sub_dat$name )
# remove empty levels
sub_dat$name <- factor(sub_dat$name)


sub_dat_df <- as.data.frame(sub_dat)
sub_dat$name <- as.character(sub_dat$name)
...
ecuador_cols <- tmp_ecuador[, c("name", "colours")]
ecuador_cols <- ecuador_cols[duplicated(ecuador_cols)==F,]
ecuador_cols$name<- sub('\\_.*', '', ecuador_cols$name)

ecuador_cols$name <- as.character(ecuador_cols$name)

night_shp <- left_join(night_shp, ecuador_cols)

#night_dat_df <- left_join(night_dat_df, ecuador_cols, by=c("name"= "name"))
first_and_last <- left_join(first_and_last, ecuador_cols, by=c("name"= "name"))
...
Average horizontal location accuracy:
...
yform <- list(categoryorder = "array",
              categoryarray = unique(tmp_ecuador$name))
